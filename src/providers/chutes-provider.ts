...
export class ChutesProvider implements IAIProvider {
  ...
  async createCompletion(request: Omit<ChatCompletionRequest, 'prompt'> & { messages: MessageContent[] }): Promise<ChatCompletionResponse> {
    try {
      Logger.info(`${this.getProviderName()} | Creating completion with model: ${request.model || 'zai-org/GLM-4.5-FP8'}`);
      
      const completion = await this.client.chat.completions.create({
        messages: this.formatMessages(request.messages),
        model: request.model || 'zai-org/GLM-4.5-FP8',
